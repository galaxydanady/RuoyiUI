    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['stu:stud:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="studList" @selection-change="handleSelectionChange">
...
<script>
import { listStud, getStud, delStud, addStud, updateStud } from "@/api/stu/stud";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listStud(this.queryParams).then(response => {
        this.studList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getStud(studentId).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.studentId != null) {
            updateStud(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addStud(this.form).then(response => {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('stu/stud/export', {
        ...this.queryParams
      }, `stud_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
